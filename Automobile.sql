...
SELECT Make, Model, Price / Cylinders AS 'Price Per Cylinder'
FROM Cars
...
SELECT Make, Model, Year, MAX(Price) AS MaxPrice
FROM Cars
GROUP BY Year
...
SELECT Make, Model, Price,
       ROW_NUMBER() OVER(ORDER BY Make, Model) AS RowNum,
       AVG(Price) OVER(PARTITION BY Make, Model) AS AvgPrice
FROM Cars;
...
SELECT Make, Model, Price, Category
FROM Cars C
...
SELECT C1.Make, C1.Model, C1.Year, C1.Fuel, C1.HP, C1.Price AS Price1, 
C2.Price AS Price2, ABS(C1.Price - C2.Price) AS Price_difference
FROM Cars C1
JOIN Cars C2 ON C1.Make = C2.Make AND C1.Model = C2.Model
...
SELECT Make, AVG(Price) AS AvgPrice, COUNT(*) AS TotalCount
FROM Cars
GROUP BY Make
...
SELECT A.Make, A.Model, B.Make AS Other_Make, B.Model AS Other_Model
FROM Cars A
CROSS JOIN Cars B
WHERE A.Make <> B.Make OR A.Model <> B.Model;



# Running total of car prices ordered by year
SELECT Make, Model, Year, Price,
       SUM(Price) OVER (PARTITION BY Year ORDER BY Make, Model) AS RunningTotal
FROM Cars;
